 ...
 * Version: 1.0
 * 
 * Description: Test hsearch on a non empty hash table.
 * 
 */
...
int main(void){
    ...
    person_t *check = (person_t*)hsearch(htp, &search_name, "Girth", sizeof("Girth"));
	if ( check != &person3 ) {
        printf("Test Failed.\n");
        //printf("Check: %p", check);
        printf("Expected: %d & %lf / Actual: %d & %lf\n",
                person3.age, person3.rate, check->age, check->rate);
        hclose(htp);
        exit(EXIT_FAILURE);
    }
